...
async function handleZernioCallback() {
    ...
    // Enhanced callback mapping to respect structural parameters passed back from Zernio
...
async function connectPlatform(platform) {
    ...
    try {
        // Step 1: Check to see if we have an existing verified profile container string cached
        let targetProfileId = window.zernioProfileId;

        if (!targetProfileId) {
            console.log("No Zernio profile located for this operator. Launching dynamic provisioning sequence...");
            
            // Call your secure backend bridge endpoint to run the server-to-server POST to Zernio
            const provisionResponse = await fetch(`${backendBaseUrl}/api/create-zernio-profile`, {
                method: "POST",
                headers: {
            ...
            if (!provisionResponse.ok) {
                ...
                throw new Error("Workspace creation failed: " + errText);
            }

            const provisionData = await provisionResponse.json();
            targetProfileId = provisionData.zernio_profile_id;
        ...
        const response = await fetch(`${backendBaseUrl}/api/get-connect-url?platform=${platform}&profileId=${targetProfileId}`);
        const data = await response.json();
